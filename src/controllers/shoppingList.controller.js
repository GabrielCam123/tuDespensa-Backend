...
/////////////////////////////////////WEB/////////////////////////////////////
export const getListaByUserId = async (req, res) => {
  const { id } = req.params;
  try {
    const lista = await ShoppingList.findOne({ user: id });
    if (!lista) {
      return res
        .status(404)
        .json({ message: "Lista de compras no encontrada para este usuario" });
    }
    res.json(lista);
  } catch (error) {
    res

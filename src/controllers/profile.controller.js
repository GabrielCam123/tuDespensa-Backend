...
async function safeCleanup(filePath, maxAttempts = 3) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      console.log(
        `Intento ${i + 1} de eliminar ${filePath} falló, reintentando...`
      );
      if (i === maxAttempts - 1) {
        console.log(
          `No se pudo eliminar ${filePath}, se limpiará en el próximo reinicio`
        );
...
const fileFilter = (req, file, cb) => {
  try {
    ...
    if (file.mimetype.startsWith("image/") || file.mimetype === "image/webp") {
      ...
      cb(
        new Error(
          "El archivo debe ser una imagen (jpg, jpeg, png, gif o webp)"
        ),
        false
      );
    }
...
export const uploadProfilePhoto = async (req, res) => {
  ...
  try {
    ...
    if (!req.user || !req.user.id) {
      return res
        .status(401)
        .json({ message: "Usuario no autenticado o ID no disponible" });
    }
    ...
    tempFilePath = path.join(
      ...
      "..",
      "imgsUsr",
      `temp_${Date.now()}_${req.file.filename}`
    );
